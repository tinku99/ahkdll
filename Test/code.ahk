; Codegen by Lexikos modified by Naveen for ahkdll
...
code_process_blocks(start_line, parent_line=0, one_line_only=false)
{
    ...
    Loop
    {
        ...
        ; IF, LOOP or REPEAT
        if (line_action >= 10 && line_action <= 33) || line_action = 104 || line_action = 8
        {
...
code_add_label(label_ptr, first_label=0)
{
    ...
    NumPut(NumGet(label_list+0), first_label+12)
    return true
}
...
code_delete_var(var_ptr)
{
    if NumGet(var_ptr+16) ; mCapacity
        alias(ref, var_ptr+0), VarSetCapacity(ref, 0), alias(ref, 0)
