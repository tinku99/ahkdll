; LowLevel by Lexikos modified by Naveen for ahkdll
...
getVarInContext(sVarName, pScopeFunc=0)
{
    static pThisFunc
...
; WARNING:
    ; If the thread is interrupted at this point and getVarInContext is called
    ; again, the wrong set of local variables will be backed up and restored, so
    ; the second instance of getVarInContext will overwrite the local vars
    ...
    ; (This would not work fully if getVar were implemented in script.)
    
    ; Now resolve %sVarName% in the scope of the other func!
    , pVar := getVar(%sVarName%)
...
__expr(expr, pScopeFunc=0)
{
    ...
    ; Using static ensures the Line is never deleted. Attempting to view a
    ...
    VarSetCapacity(Line%nInst%,44,0), static(Line%nInst%), pLine:=&Line%nInst%
...
__malloc(size)
{
    ...
    VarSetCapacity(v,size-1),p:=&v,NumPut(0,NumPut(0,NumPut(&empty,getVar(v),8)))
    return p
}
...
__free(ptr)
{
    ...
    NumPut(2,NumPut(1,NumPut(ptr,getVar(v),8),4),0,"char")
